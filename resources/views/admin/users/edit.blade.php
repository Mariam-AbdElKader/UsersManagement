<x-admin.layout title="Edit User #{{ $user->id }}">
    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="bg-gray-800 p-6 rounded-lg shadow-lg max-w-3xl mx-auto">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-6">
            <label for="is_admin" class="inline-flex items-center text-gray-300">
                ...
                <input type="checkbox" name="is_admin" id="is_admin" {{ old('is_admin', $user->is_admin) ? 'checked' : '' }} value="1" class="form-checkbox h-5 w-5 text-purple-600">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" class="px-6 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-md shadow hover:scale-105 transition">Update User</button>
        </div>
    </form>
</x-admin.layout>
